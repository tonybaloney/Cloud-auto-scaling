...
class Alerts { 
	 ...
	 **/
	 public static function TriggerScalingAlert ($customerId,$clusterId, $triggerId, $action, $approval){
		...
		if ($action=='SCALE_UP')
			$val = DB::GetRecord("SELECT AVG(result) FROM tick_log WHERE triggerId=$triggerId AND date > SUBDATE(date,INTERVAL ".$trigger->scaleUpTime." SECOND);" ) ;
		else 
			$val = DB::GetRecord("SELECT AVG(result) FROM tick_log WHERE triggerId=$triggerId AND date > SUBDATE(date,INTERVAL ".$trigger->scaleDownTime." SECOND);" ) ;
